...
{
    [RequireComponent(typeof(Interfaces.IFuselageBuilder))]
    public class FuselageRandomizerTag : RandomizerTag
    {
        private Interfaces.IFuselageBuilder builder = null;
        private void Awake()
        {
            builder = gameObject.GetComponent<Interfaces.IFuselageBuilder>();
        }
        public void BuildFuselage(Matrix2D m) => builder.Build(m);
        public void DestroyFuselage() => builder.Crush();
        public int MtxDimX => builder.CountHoleBlocksX;
        public int MtxDimY => builder.CountHoleBlocksY;
